...
public class EllerMazeGenerator
{
    ...
    {
        Data = new DataProvider(Width, Height);
        ...
        {
            DataLogic.FillGroupsRow(i);
            GenerateRightBorders(i);
            GenerateBottomBorders(i);
            DataLogic.FlowRow(i);
            //FillGroups(i);
        ...
        DataLogic.FillGroupsRow(Height - 1);
        GenerateRightBorders(Height - 1);
        GenerateBottomBorders(Height - 1);
    ...
    void GenerateRightBorders(int row)
    {
        row *= Height;
        DataLogic.PreventDuplicates(row, row + Width - 1);
        for (int i = 0; i < Width; i++)
            if (BetweenColumnBorder())
                Data.InsertRightBorder(row + i);

        Data.InsertRightBorder(row + Width - 1);
    }
    void GenerateBottomBorders(int row)
    {
        row *= Height;
        for (int i = 0; i < Width; i++)
            if (BetweenRowBorder())
                Data.InsertBottomBorder(row + i);
        DataLogic.EliminateClosedCells(row, Width);
    }
